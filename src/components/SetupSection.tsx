        ...
        <section id="setup" className="py-32 px-4 w-full bg-background relative">
            <div className="max-w-6xl mx-auto">
                <div className="mb-16 md:mb-24 text-center">
                    ...
                    <div className="h-1.5 w-24 bg-teal-500 rounded-full mt-2 mx-auto"></div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    {/* Peripherals */}
                    <div className="bg-card/50 border border-border rounded-3xl p-8 backdrop-blur-sm hover:border-teal-500/30 transition-colors shadow-lg">
                        <h3 className="text-2xl font-bold text-teal-400 mb-6 flex items-center gap-3">
                            <i className="fas fa-gamepad"></i> Peripherals
                        </h3>
                        <ul className="space-y-4">
                            {hardware.map((hw, idx) => (
                                <li key={idx} className="flex items-center justify-between border-b border-border pb-3 last:border-0 last:pb-0">
                                    <span className="flex items-center gap-3 text-foreground/80">
                                        <i className={cn(hw.icon, "w-6 text-center text-muted-foreground")}></i>
                                        {hw.item}
                                    </span>
                                    <span className="text-muted-foreground text-sm font-light text-right">{hw.detail}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div className="bg-card/50 border border-border rounded-3xl p-8 backdrop-blur-sm hover:border-teal-500/30 transition-colors shadow-lg">
                        ...
                        <ul className="space-y-4">
                            {pcSpecs.map((spec, idx) => (
                                <li key={idx} className="flex items-center justify-between border-b border-border pb-3 last:border-0 last:pb-0">
                                    <span className="flex items-center gap-3 text-foreground/80">
                                        <i className={cn(spec.icon, "w-6 text-center text-muted-foreground")}></i>
                                        {spec.item}
                                    </span>
                                    <span className="text-muted-foreground text-sm font-light text-right">{spec.detail}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
